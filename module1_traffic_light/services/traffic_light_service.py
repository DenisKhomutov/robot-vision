...
def analyze(image: Image.Image) -> dict[str, Any]:
    ...
    if label == config.UNKNOWN_LABEL or conf < config.CLS_CONF:
        return {"status": "light_unclassified", "signal": None, "confidence": round(conf, 4), "box": list(box)}
    # светофор найден и сигнал распознан
    return {"status": "light_classified", "signal": label, "confidence": round(conf, 4), "box": list(box)}
