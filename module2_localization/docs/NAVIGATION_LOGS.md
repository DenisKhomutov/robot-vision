...
затем `_003` и так далее. Старые части не удаляются. После полуночи автоматически
начинается файл с новой датой. Перезапуск systemd в пределах дня продолжает текущий
файл, а не создаёт новый; каждая строка содержит `pid`, а границы запусков отмечены
`session_started` и `session_stopped`.
