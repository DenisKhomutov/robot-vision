...
async def worker(front_src, rear_src, nc, topic: str, nav, stop_evt=None, traffic=None,
    ...
    while not (stop_evt and stop_evt.is_set()):
        ...
        if nav.mode == "front":
            ...
            candidate = (("dual", front_stamp, rear_stamp)
                         if fframe is not None and rframe is not None else None)
            if (candidate is not None and last_active_token is not None
                    and last_active_token[0] == "dual"
                    and (candidate[1] == last_active_token[1]
                         or candidate[2] == last_active_token[2])):
        ...
        if fresh_active_frame:
            ...
            if navlog is not None:
                navlog.emit("frame_received", source_stamp=active_token,
                            front=frame_metrics(fframe), rear=frame_metrics(rframe),
                            route=nav.route, mode=nav.mode, frame_guard=frame_state)
            ...
            if route_profile is not None:
                ...
                if navlog is not None:
                    navlog.emit("command_after_route_profile", source_stamp=active_token,
                                command=dict(cmd))
            if direction is not None:
                direction.process(cmd)
                if navlog is not None:
                    navlog.emit("command_after_direction", source_stamp=active_token,
                                command=dict(cmd))
            ...
            if traffic and fframe is not None:
                ...
                if navlog is not None:
                    navlog.emit("command_after_traffic", source_stamp=active_token,
            ...
            if navlog is not None:
                navlog.emit("command_published", source_stamp=active_token, topic=topic,
            ...
            cmd = {
                "move_type": "stop", "deg": 0.0, "reason": "camera_timeout",
                "route_loaded": nav.route is not None, "route": nav.route,
                "map": nav.front_map if nav.mode == "front" else nav.rear_map,
                "mode": nav.mode, "cam": nav.mode,
                "paused": bool(nav.pf.paused and nav.pr.paused),
                "frame_guard_enabled": True, "frame_timed_out": True,
            }
            if route_profile is not None:
                route_profile.process(cmd)
            if direction is not None:
            ...
            if nc:
                await nc.publish(topic, payload)
            if navlog is not None:
                navlog.emit("camera_timeout", source_stamp=active_token,
                            frame_guard=frame_state, command=dict(cmd))
                navlog.emit("command_published", source_stamp=active_token, topic=topic,
                            nats=nc is not None, command=dict(cmd))
...
async def main() -> int:
    ...
    diagnostic_config_names = (
        ...
        "FRAME_TIMEOUT_CHECK_ENABLED", "FRAME_TIMEOUT_S", "FRAME_TIMEOUT_RECOVERY_FRAMES",
    )
    navlog.emit(
        "navigation_config",
